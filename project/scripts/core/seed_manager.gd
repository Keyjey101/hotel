...
## WARNING: Godot's built-in hash() is NOT cross-platform deterministic.
## Different OS or Godot versions may produce different results for the same
## input. For true cross-platform determinism, replace hash() calls with a
## custom deterministic hash (e.g. FNV-1a, MurmurHash3).
...
func get_room_enemy_config(floor_number: int, room_index: int, spawn_points: int) -> Dictionary:
	...
	# Use a derivative RNG so repeated calls return the same results
	var enemy_rng := RandomNumberGenerator.new()
	enemy_rng.seed = hash(str(rng.seed) + "_enemy")
	var count := enemy_rng.randi_range(ceili(spawn_points * 0.6), spawn_points)
	...
	_deterministic_shuffle(all_points, enemy_rng)
