...
func setup(weapon: WeaponData, direction: Vector2, damage_mult: float = 1.0) -> void:
	...
	freeze = false
	gravity_scale = 1.0
	# Clean up any ColorRect children added by bleed/embed effects on previous use
...
func _on_body_entered(body: Node2D) -> void:
	...
	if body.is_in_group("enemy"):
		...
		if not visible:
			return
		if _weapon and _weapon.throw_effect == "bounce":
			return  # Bounce already handled velocity
...
func _return_to_pool() -> void:
	# Clean up any ColorRect children from bleed/embed effects
	for child in get_children():
		if child is ColorRect:
			child.queue_free()
	...
	# Disconnect all hit signal connections to release closures holding WeaponData references
	...
	# Clear weapon reference to break closure references
	_weapon = null
...
func _apply_stick_bleed() -> void:
	...
	if is_instance_valid(get_tree().current_scene): get_tree().current_scene.add_child(zone)
...
func _apply_embed(target: Node2D) -> void:
	...
	if target is CharacterBody2D:
		...
		if "move_speed" in target:
			...
			var _embed_timer := get_tree().create_timer(7.0, true, false, true)
			_embed_timer.timeout.connect(func():
...
func _apply_demoralize() -> void:
	...
	ring.z_index = 10
	if is_instance_valid(get_tree().current_scene): get_tree().current_scene.add_child(ring)
...
func _apply_tangle(target: Node2D) -> void:
	...
	wire.z_index = 5
	if is_instance_valid(get_tree().current_scene): get_tree().current_scene.add_child(wire)
...
func _apply_soul_rip(target: Node2D) -> void:
	...
	if randf() < 0.25:
		...
		particle.z_index = 10
		var _target_pos := global_position
		if is_instance_valid(get_tree().current_scene): get_tree().current_scene.add_child(particle)
		var tween := particle.create_tween()
		tween.tween_property(particle, "global_position", _target_pos, 0.4)
...
func _apply_reality_tear() -> void:
	...
	circle.z_index = 10
	if is_instance_valid(get_tree().current_scene): get_tree().current_scene.add_child(circle)
...
func _discharge() -> void:
	...
	var directions := [Vector2.UP, Vector2.DOWN, Vector2.LEFT, Vector2.RIGHT,
		...
		Vector2(-1, 1).normalized(), Vector2(-1, -1).normalized()]
	var count := 1
	if _weapon.weapon_id == "smg":
		count = 5
	elif _weapon.weapon_id == "shotgun":
		count = 8

	for i in range(count):
		var dir = directions[randi() % directions.size()]
		# Try to use the projectile object pool via WeaponManager → projectile pool
		var proj: CharacterBody2D = null
		var player := get_tree().get_first_node_in_group("player")
		if player and player.has_node("WeaponManager"):
			var wm := player.get_node("WeaponManager")
			if wm.get("_projectile_pool") != null:
				proj = wm._projectile_pool.get_instance()
		if proj == null:
			proj = preload("res://scenes/weapons/projectile.tscn").instantiate()
		...
		# Reparent to scene if coming from pool
