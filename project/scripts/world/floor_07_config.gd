...
static func _on_camera_hit(area: Area2D, cam: Area2D, room: RoomInstance) -> void:
	...
	# Only react to attack areas (melee hits, projectiles)
	var area_name: String = area.name
	if not area_name.begins_with("Melee") and not area_name.begins_with("Projectile") and not area.is_in_group("player_attack"):
		return
